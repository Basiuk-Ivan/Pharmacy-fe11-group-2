    ...
    return (
        <Search>
            ...
            {text !== '' && (
                ...
                >
                    <Stack divider={<Divider orientation="horizontal" sx={{color: "black"}} flexItem/>}
                           sx={{maxWidth: 360, maxHeight: 310, overflowY: 'auto', minHeight:100}}>
                        {products.length>0 ? products.map(item => (
                            <NavLink key={item.id} to={`/${item?.categories[0]}/${item?.id}`}>
                                <Stack key={item.id} direction="row" justifyContent="flex-start" alignItems="center"
                                       spacing={1}>
                            ...
                            (<Box sx={{margin: "auto", fontFamily: 'Roboto, sans-serif', textAlign:"center",
                                fontSize: "16px", color: "#011d71"}}>За даним запитом нічого не знайдено. Уточніть свій запит.</Box>)
                       }
                    </Stack>
                </Box>
            )}
            {text !== '' && <ClearIcon onClick={()=>{setText("")}}
